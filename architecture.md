...
A zero-trust security interceptor designed to prevent Server-Side Request Forgery.
- **Host Resolution**: Resolves IP addresses before firing actual HTTP/HTTPS network calls.
...
- **`http`**: Node's native `fetch` agent. Optimizes speed and minimal memory footwork for basic HTML sites.
- **`stealth` / `rendered`**: Playwright chromium integration. The stealth engine emulates system plugins, mimics WebGL details, and disables automatic WebDriver indicators to bypass anti-scraping layers.
- **`fast`**: Spawns a native compiled Rust subprocess (`obscura`) designed for extremely high-frequency static HTML scrapes.
