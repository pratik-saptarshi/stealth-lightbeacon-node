...
export class UxEvaluator implements Evaluator {
  readonly id = 'ux';
  readonly domain = 'UX';
  ...
  async evaluate(context: EvaluationContext): Promise<DomainResult> {
    ...
    const getListDepth = (el: any): number => {
      let maxSubDepth = 0;
      el.children('li').each((_: any, li: any) => {
        $(li).children('ul, ol').each((_: any, subList: any) => {
    ...
    let maxNavDepth = 0;
    $('nav, [class*="menu"]').each((_: any, container: any) => {
      $(container).find('> ul, > ol').each((_: any, rootList: any) => {
